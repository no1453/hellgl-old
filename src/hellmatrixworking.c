...
static void draw_text1(void)
{
   ...
   long p=0,b=0;
   int c,c_pic;
   if(!paused && pic_mode==1 && (pic_fade+=3)>255)
      pic_fade=255;
   if(!paused && pic_mode==2 && (pic_fade-=3)<0)
      pic_fade=0;
   for(y=_text_y;y>-_text_y;y--){
      for(x=-_text_x;x<_text_x;x++){
	    ...
	    c=255-c;
            /* 3D Image offsets */
            c_pic=255-(pic[b+pic_offset])-(255-pic_fade); 
            if (c_pic<0)
               c_pic=0;
            c+=c_pic;
            if(c>255)
            ...
            if(!paused)bump_pic[p]=(float)(255-c_pic)/64;

            if (text[p] && c!=0 )
              draw_char(1, (text[p])%NUM_CHARS,c,x,y, 8+bump_pic[p]);
           ...
           if (c>255)
              c=255;
           draw_char(1, (text[p])%NUM_CHARS,255, x ,y, 8+8);
...
static void draw_text2(int mode)
{
   ...
   for(y=_text_y;y>-_text_y;y--){
      for(x=-_text_x;x<_text_x;x++){
         if(text_light[p]>245 && text_light[p+text_x]<128 ) {
            /* White character */
            if (!mode) {
               draw_char(2, (text[p]+1)%NUM_CHARS,0.5,x,y, 
...
static void scroll(int mode)
{
   ...
   for (line=1; line<text_y; line++) {
      memcpy(text_light+(line-1)*text_x ,text_light+line*text_x, text_x);
   }
/*   for (i=0; i<density; i++) {*/
   ...
   if (odd) {
	  ...
	  if(timer>75 && timer<80 && !classic) pic_mode=2; /* pic fade out */
	      ...
	      pic_offset==(num_pics+2)*(rtext_x*text_y)) ) {
            ...
            timer=110;pic_mode=1; /* Make DC dissapear quickly */
         }
#ifdef NIX_MODE
      if(timer>210) {
#else /* WIN32_MODE */
